...
char *search_entry(HashDirectory *dir, int key) {
    int index = hash_function(key, dir->global_depth);
    Bucket *bucket = dir->buckets[index];
    //printf("O bucket desse dado está no %i",index); //CONFERIR SE ESTA NO HASH CERTO
    printf(" o local é %s ",dir->buckets[index]->filename);
    //DIR bucket_file = fopen (dir->buckets[index]->filename)
